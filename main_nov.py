...
for row in range(N):
    if nightshift[row] == 0:
        ...
        # senior physisians
        if competence[row] == 1:
            request_late[row, :] = -fit_var['PEN_MEDIUM']

request_night[request_night < 0] = -fit_var['PEN_HARD']
request_night[request_night > 0] = fit_var['PEN_MEDIUM']
...
def showBestPlan(individual):
    ...
    fig.savefig('vagtplan_output.png')
    plt.close(fig)
...
"""
if model_var['DEBUG']:
    np.random.seed(85)  # Perfect roster for POP_SIZE 100
    random.seed(85)     
"""
...
fit_array = fitness(best_individual, array=True)
plan, details = binaryToPlan(best_individual, fit_array)

workbook = xlsxwriter.Workbook('vagtplan_output.xlsx')
worksheet = workbook.add_worksheet()


worksheet.write_row(0, 1, dates)
worksheet.write_column(1, 0, names)

row = 1

for col in range(1, M+1):
    worksheet.write_column(row, col, plan[:,col-1])
